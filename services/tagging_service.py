...
TAG_DESCRIPTIONS: dict[str, str] = {
    ...
    "#мета": (
        "мета-обсуждение самого канала/сообщества «Косплей-гримерка ЕКБ»: правила, формат контента, "
        "модерация, админы, публикации, политика канала, внутренняя жизнь канала; "
        "не ставить для обычной 'гримерки' как физического места/комнаты без контекста канала"
    ),
    "#наброс": (
        "негативный/конфликтный пост про конкретную личность или персональный наезд: "
        "обвинения, хейт, токсичные претензии, попытка очернить человека по имени/нику; "
        "не ставить для нейтральной критики явления без персональной атаки"
    ),
    "#тейк": (
    ...
    "#расхламовка": (
        "объявление о продаже, отдаче, даре, обмене или расхламовке вещей/реквизита/материалов: "
        "'продам', 'продаю', 'отдам', 'даром', 'заберите', 'меняю', 'ищу нового хозяина'; "
        "не ставить, если человек сам ищет/хочет купить вещь (это #вопросы или #помощь)"
    ),
}
...
class TaggingService:
    ...
    def _apply_heuristics(
        ...
        channel_meta_markers = [
            "косплей-гримерка",
            "косплей гримерка",
            "гримерка екб",
            "гримерка екатеринбург",
            "наш канал",
            "в канале",
            "админы",
            "модерац",
        ...
        channel_meta_hits = sum(1 for marker in channel_meta_markers if marker in text)
        ...
        declutter_hits = sum(1 for marker in declutter_markers if marker in text)
        ...
        # Усиливаем #мета, когда есть явный контекст обсуждения канала/сообщества.
        if channel_meta_hits >= 2:
            current_meta = score_map.get("#мета", 0.0)
            score_map["#мета"] = max(current_meta, 8.2)

        # Усиливаем #расхламовка для явных объявлений о продаже/отдаче/обмене.
        if declutter_hits >= 1:
            current_declutter = score_map.get("#расхламовка", 0.0)
            score_map["#расхламовка"] = max(current_declutter, 8.5)
    ...
    async def score_tags(self, content_text: str) -> list[TagScore]:
        ...
        prompt = (
            ...
            "- #мета ставь только при обсуждении канала/сообщества «Косплей-гримерка ЕКБ» (админы, модерация, правила, формат канала, публикации, внутренняя кухня сообщества).\n"
            "- Если в тексте просто упоминается гримерка как место/комната/локация без контекста канала, #мета не ставить.\n"
            "- #наброс ставь, если это негативный пост/наезд на конкретную личность (имя, ник, явная персональная атака).\n"
            ...
            "- #расхламовка ставь для объявлений о продаже, отдаче, даре, обмене или расхламовке вещей/реквизита; "
            "не путай с #вопросы (человек сам ищет/хочет купить) и #помощь (просьба помочь).\n"
